...
module.exports = class Artist extends Command {
	constructor() {
		super('artist', {
			aliases: ['artist', 'creator'],
			category: 'Utility',
			description: {
				content: 'Gives Artist role to the user.',
				usage: '.artist <user>',
				examples: [''],
			},
			slash: true,
	...
	async exec(message, args) {
		const data = await this.client.database.getRole(message.guild.id, 'artist');
		...
		if (!data) {
			await message.util.send({
				content: 'Artist Role is not setupped in this server!Try `.rolesetup artist <role>',
			});
		...
		if (!role) {
			await message.util.send({
				content: 'Artist Role is not present in this server!',
			});
